...
class ButterflyNetSimpleRNN(nn.Module):
    ...
    def init_hidden(self, batch_size, input_size):
        ...
        hidden_states = [
            Variable(torch.zeros(batch_size, 64, 20, 20,
                                 requires_grad=True,
                                 device=torch.device('cuda'))),
            Variable(torch.zeros(batch_size, 64, 20, 20,
                                 requires_grad=True,
                                 device=torch.device('cuda'))),
            Variable(torch.zeros(batch_size, 128, 20, 20,
                                 requires_grad=True,
                                 device=torch.device('cuda'))),
        ]
