...
def dice_score_list(label_gt, label_pred, n_class):
    ...
    for batch_id, (l_gt, l_pred) in enumerate(zip(label_gt, label_pred)):
        for class_id in range(n_class):
            ...
            score = 2.0 * np.sum(img_A * img_B) / \
                (np.sum(img_A) + np.sum(img_B) + epsilon)
...
def precision_and_recall(label_gt, label_pred, n_class):
    ...
    precision[:] = precision_score(
        img_A, img_B, average=None, labels=range(n_class))
...
def distance_metric(seg_A, seg_B, dx, k):
    ...
    for z in range(Z):
        ...
        if np.sum(slice_A) > 0 and np.sum(slice_B) > 0:
            ...
            md = 0.5 * (np.mean(np.min(M, axis=0)) +
                        np.mean(np.min(M, axis=1))) * dx
            hd = np.max([np.max(np.min(M, axis=0)),
                        np.max(np.min(M, axis=1))]) * dx
            table_md += [md]
            table_hd += [hd]
